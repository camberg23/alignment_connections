...
# Removed additional angles from the summarizer since it's no longer needed there
...
def chat_interface(key, base_content, model, verbose):
    # base_content is the main content shown in expander
    # We'll maintain session state of messages for this expander
    if f"chat_{key}_messages" not in st.session_state:
        st.session_state[f"chat_{key}_messages"] = []

    # Display existing chat messages
    for msg in st.session_state[f"chat_{key}_messages"]:
        if msg["role"] == "user":
    ...
    if prompt:
        # User message
        st.session_state[f"chat_{key}_messages"].append({"role":"user","content":prompt})
        # Call LLM with base_content + conversation
        # We'll send the entire conversation + base content for context
        # Simple approach: system message with instructions + user role with all messages
        ...
        for m in st.session_state[f"chat_{key}_messages"]:
            conv_messages.append(m)
        response = run_completion(conv_messages, model, verbose)
        st.session_state[f"chat_{key}_messages"].append({"role":"assistant","content":response})
        with st.chat_message("assistant"):
            st.write(response)

def download_expander_content(label, content):
    # content is a string, we convert to markdown
    # The user may want all chat as well. Let's include chat logs
...
if st.button("Run Pipeline"):
    ...
    if not final_input_text:
        ...
        with st.expander("Comprehensive Systematic Overview") as overview_exp:
            st.write(overview)
        ...
        with st.expander("Initial Alignment Ideas") as ideas_exp:
        ...
        while iteration_count < max_iterations:
            ...
            with st.expander(f"Iteration {iteration_count+1} Critique") as critique_exp:
                st.write(critique_text)
                download_expander_content(f"iteration_{iteration_count+1}_critique", critique_text)
                chat_interface(f"iteration_{iteration_count+1}_critique", critique_text, ideator_model, verbose)
            ...
            with st.expander(f"Iteration {iteration_count+1} Internal Reasoning (CRITIQUE_START/END)") as internal_exp:
                st.write(critique_section)
                download_expander_content(f"iteration_{iteration_count+1}_internal_reasoning", critique_section)
                chat_interface(f"iteration_{iteration_count+1}_internal_reasoning", critique_section, ideator_model, verbose)

            with st.expander(f"Iteration {iteration_count+1} Refined Directions") as refined_exp:
                st.write(improved_ideas)
                download_expander_content(f"iteration_{iteration_count+1}_refined_directions", improved_ideas)
                chat_interface(f"iteration_{iteration_count+1}_refined_directions", improved_ideas, ideator_model, verbose)
        ...
        if final_good_enough:
            with st.expander("Final Accepted Ideas", expanded=True) as final_exp:
                st.write(current_ideas)
                download_expander_content("final_accepted_ideas", current_ideas)
                chat_interface("final_accepted_ideas", current_ideas, ideator_model, verbose)
        else:
            ...
            with st.expander("Final Non-Approved Ideas") as final_non_approved_exp:
                st.write(current_ideas)
                download_expander_content("final_non_approved_ideas", current_ideas)
                chat_interface("final_non_approved_ideas", current_ideas, ideator_model, verbose)

            assessment = assess_strengths_weaknesses(current_ideas, terminator_model, verbose)
            with st.expander("Strengths & Weaknesses Assessment", expanded=True) as swa_exp:
                st.write(assessment)
