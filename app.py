...
# st.write("You may upload a PDF or paste text, as chosen below. Within each expander, you can chat with the LLM (the same model used to generate that content) to further explore and refine the displayed content. You can also download the contents of the expander as markdown at any time.")
...
#     # After displaying messages, offer download of entire conversation (base + chat)
#     all_chat_text = "## Base Content\n\n" + base_content + "\n\n## Conversation\n\n"
#     for m in st.session_state.conversation_states[key]:
#         role_name = "User" if m["role"]=="user" else "Assistant"
#         all_chat_text += f"**{role_name}:** {m['content']}\n\n"

#     download_expander_content(f"{key}_conversation", all_chat_text)
...
# input_method = st.radio("Choose input method:", ["Upload PDF","Paste Text"], index=1)
...
# else:
#     pass  # We'll use user_text below

# show_additional_angles = st.checkbox("Optional additional angles or considerations for alignment directions:")
# if show_additional_angles:
#     additional_context = st.text_area("Provide additional angles or considerations to steer alignment directions:", height=100)
# else:
#     additional_context = ""

# llm_options = ["gpt-4o", "o1-mini", "o1-2024-12-17", "anthropic:claude"]

# summarizer_model = st.selectbox("Summarizer Model:", llm_options, index=0)  # gpt-4o
# ideator_model = st.selectbox("Ideator Model:", llm_options, index=1)  # o1-mini
# critic_model = st.selectbox("Critic Model:", llm_options, index=3)    # anthropic:claude
# terminator_model = st.selectbox("Terminator Model:", llm_options, index=0)  # gpt-4o

# verbose = st.checkbox("Show verbose debug info")

# max_iterations = st.number_input("Max Refinement Iterations:", min_value=1, value=5)
...
#         chat_interface("overview", overview, summarizer_model, verbose)
...
#             chat_interface(f"iteration_{i}_critique", critique_text, critic_model, verbose)
...
#             # This internal reasoning was produced by ideator model (re_ideate step)
...
#             # Refined directions also produced by the ideator model
...
#             chat_interface("final_accepted_ideas", final_ideas, terminator_model, verbose)
#     else:
...
#             chat_interface("final_non_approved_ideas", final_ideas, terminator_model, verbose)
...
#                 # Assessment was produced by terminator model
#                 chat_interface("strengths_weaknesses_assessment", assessment, terminator_model, verbose)
